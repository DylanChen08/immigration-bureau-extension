# 移民局扩展 - 核心功能与技术文档

## 📋 项目概述

这是一个 Chrome 浏览器扩展，用于在移民局网站表单页面显示全屏遮罩提示。当用户进入入境卡填报表单的第一步（证件资料页上传）时，自动显示"请刷证件"的提示界面。

## 🎯 核心功能

### 1. 智能页面检测
- **URL 检测**: 自动检测目标页面 URL (`entry-registation-form`)
- **步骤状态检测**: 检测 Element UI Steps 组件的第一步是否激活
- **实时监听**: 使用 MutationObserver 监听 DOM 变化，实时响应步骤切换

### 2. 全屏遮罩层显示
- **自动显示**: 当第一步激活时自动显示遮罩
- **视觉效果**: 半透明黑色背景，居中显示提示文字和动画 GIF
- **自动隐藏**: 当离开第一步时自动隐藏遮罩

### 3. 动画效果
- **文字动画**: "请刷证件"文字带有脉冲动画效果
- **GIF 动画**: 支持显示证件扫描相关的 GIF 动画
- **过渡效果**: 淡入淡出、缩放等平滑过渡动画

## 🏗️ 技术架构

### 文件结构

```
项目根目录/
├── src/
│   ├── manifest.json       # 扩展配置文件 (Manifest V3)
│   ├── content.ts          # 内容脚本（核心逻辑）
│   ├── content.css         # 遮罩层样式
│   ├── background.ts       # 后台服务脚本
│   ├── assets/             # 资源文件目录
│   │   └── id-card-scan.gif # GIF 动画文件
│   └── icons/              # 扩展图标
```

### 核心组件

#### 1. Content Script (`content.ts`)
主要的业务逻辑实现，包含：
- 页面检测逻辑
- 遮罩层创建与管理
- DOM 监听机制
- 消息通信处理

#### 2. dan shi (`content.css`)
遮罩层的完整样式定义，包括：
- 全屏定位
- 居中布局
- 动画效果
- 响应式设计

#### 3. Manifest 配置 (`manifest.json`)
Manifest V3 配置，定义：
- 扩展权限
- 内容脚本注入规则
- 资源访问配置

## 📝 核心逻辑详解

### 1. 页面检测逻辑

```typescript
function checkAndShowOverlay() {
  const currentUrl = window.location.href;
  
  // 1. URL 检测
  if (!currentUrl.includes("entry-registation-form")) {
    return;
  }

  // 2. 等待页面加载
  if (document.readyState !== "complete") {
    window.addEventListener("load", checkAndShowOverlay);
    return;
  }

  // 3. 等待 Vue 组件渲染（延迟检测）
  setTimeout(() => {
    // 4. 查找步骤元素
    const steps = document.querySelectorAll(".el-step");
    if (steps.length === 0) {
      setTimeout(checkAndShowOverlay, 500);
      return;
    }

    // 5. 检测第一步状态
    const firstStep = steps[0];
    const isFirstStepActive = 
      firstStep?.querySelector(".is-process") !== null ||
      firstStep?.querySelector(".el-step__head.is-process") !== null ||
      firstStep?.classList.contains("is-process");

    // 6. 显示或隐藏遮罩
    if (isFirstStepActive) {
      showOverlay();
    } else {
      hideOverlay();
    }
  }, 1000);
}
```

**关键点：**
- **延迟检测**: 使用 `setTimeout` 等待 Vue/Element UI 组件渲染
- **多重检测**: 通过多种选择器组合确保准确检测第一步状态
- **重试机制**: 如果元素未找到，自动重试检测

### 2. DOM 监听机制

```typescript
function observeStepChanges() {
  const stepsContainer = document.querySelector(".el-steps");
  if (!stepsContainer) {
    setTimeout(observeStepChanges, 500);
    return;
  }

  // 创建 MutationObserver 监听 DOM 变化
  const observer = new MutationObserver(() => {
    checkAndShowOverlay();
  });

  // 监听配置
  observer.observe(stepsContainer, {
    childList: true,      // 监听子节点变化
    subtree: true,        // 监听所有后代节点
    attributes: true,     // 监听属性变化
    attributeFilter: ["class"],  // 只监听 class 属性
  });

  checkAndShowOverlay();  // 初始检查
}
```

**监听策略：**
- **实时响应**: MutationObserver 实时监听步骤状态变化
- **范围监听**: 监听整个 `.el-steps` 容器及其子树
- **类名监听**: 重点监听 `class` 属性变化（`is-process` 等状态类）

### 3. 遮罩层创建

```typescript
function showOverlay() {
  // 防止重复创建
  if (document.getElementById(OVERLAY_ID)) {
    return;
  }

  const overlay = document.createElement("div");
  overlay.id = OVERLAY_ID;
  overlay.innerHTML = `
    <div class="overlay-content">
      <div class="overlay-text">请刷证件</div>
      <div class="overlay-gif-container">
        <img 
          src="${chrome.runtime.getURL("assets/id-card-scan.gif")}" 
          alt="请刷证件动画"
          class="overlay-gif"
          onerror="处理GIF加载失败"
        />
      </div>
    </div>
  `;

  document.body.appendChild(overlay);
}
```

**技术要点：**
- **动态创建**: 使用 `document.createElement` 动态创建遮罩元素
- **资源路径**: 使用 `chrome.runtime.getURL()` 获取扩展资源路径
- **错误处理**: 通过 `onerror` 处理 GIF 加载失败情况
- **防重复**: 通过 ID 检查避免重复创建

### 4. URL 变化监听

```typescript
// 监听 URL 变化（适用于 Vue Router 单页应用）
let lastUrl = window.location.href;
setInterval(() => {
  const currentUrl = window.location.href;
  if (currentUrl !== lastUrl) {
    lastUrl = currentUrl;
    checkAndShowOverlay();
  }
}, 1000);
```

**适用场景：**
- 单页应用（SPA）路由变化
- 页面动态加载
- 用户前进/后退导航

### 5. 消息通信

```typescript
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message.type === "GET_PAGE_INFO") {
    const pageInfo = {
      title: document.title,
      url: window.location.href,
      timestamp: new Date().toISOString(),
      hasFirstStep: document.querySelector(".el-step .is-process") !== null,
    };
    sendResponse(pageInfo);
  } else if (message.type === "TOGGLE_OVERLAY") {
    if (message.show) {
      showOverlay();
    } else {
      hideOverlay();
    }
    sendResponse({ success: true });
  }
  return true;  // 保持消息通道开放
});
```

## 🎨 样式设计

### 遮罩层样式要点

```css
#immigration-bureau-overlay {
  position: fixed;           /* 固定定位 */
  top: 0; left: 0;          /* 覆盖整个屏幕 */
  width: 100%; height: 100%;
  background: rgba(0, 0, 0, 0.85);  /* 半透明黑色背景 */
  z-index: 999999;           /* 最高层级 */
  display: flex;              /* Flexbox 居中 */
  justify-content: center;
  align-items: center;
  animation: fadeIn 0.3s;     /* 淡入动画 */
}
```

**设计特点：**
- **全屏覆盖**: `position: fixed` 确保覆盖整个视口
- **高层级**: `z-index: 999999` 确保在最上层
- **居中布局**: Flexbox 实现完美居中
- **动画效果**: 淡入、脉冲、缩放等多种动画

### 动画效果

```css
/* 文字脉冲动画 */
@keyframes pulse {
  0%, 100% { transform: scale(1); opacity: 1; }
  50% { transform: scale(1.05); opacity: 0.9; }
}

/* GIF 容器缩放动画 */
@keyframes scaleIn {
  from { transform: scale(0.8); opacity: 0; }
  to { transform: scale(1); opacity: 1; }
}
```

## ⚙️ Manifest 配置

### 关键配置项

```json
{
  "manifest_version": 3,
  "content_scripts": [
    {
      "matches": ["<all_urls>"],
      "js": ["content.js"],
      "css": ["content.css"]
    }
  ],
  "web_accessible_resources": [
    {
      "resources": ["assets/*"],
      "matches": ["<all_urls>"]
    }
  ]
}
```

**配置说明：**
- **matches**: `["<all_urls>"]` 在所有页面注入脚本
- **web_accessible_resources**: 允许页面访问扩展资源（GIF 文件）
- **css**: 自动注入样式文件

## 🔧 在其他项目中使用

### 1. 复制核心文件

需要复制的文件：
- `src/content.ts` → 内容脚本逻辑
- `src/content.css` → 样式文件
- 相关资源文件（GIF、图标等）

### 2. 修改配置

#### 修改目标 URL 检测

```typescript
// 在 content.ts 中修改
if (!currentUrl.includes("your-target-url")) {
  return;
}
```

#### 修改步骤检测选择器

```typescript
// 根据目标网站调整选择器
const steps = document.querySelectorAll(".your-step-selector");
const isFirstStepActive = 
  firstStep?.querySelector(".your-active-class") !== null;
```

#### 修改遮罩文字和 GIF

```typescript
overlay.innerHTML = `
  <div class="overlay-content">
    <div class="overlay-text">你的提示文字</div>
    <div class="overlay-gif-container">
      <img src="${chrome.runtime.getURL("assets/your-gif.gif")}" />
    </div>
  </div>
`;
```

### 3. 集成步骤

1. **添加 Manifest 配置**
   ```json
   {
     "content_scripts": [
       {
         "matches": ["你的目标网站URL"],
         "js": ["content.js"],
         "css": ["content.css"]
       }
     ]
   }
   ```

2. **配置资源访问**
   ```json
   {
     "web_accessible_resources": [
       {
         "resources": ["assets/*"],
         "matches": ["你的目标网站URL"]
       }
     ]
   }
   ```

3. **准备资源文件**
   - 将 GIF 文件放置在 `assets/` 目录
   - 更新代码中的 GIF 路径

## 📊 工作流程图

```
页面加载
  ↓
内容脚本注入
  ↓
检测 URL 是否匹配
  ↓ 是
等待页面加载完成
  ↓
等待 Vue 组件渲染（延迟 1 秒）
  ↓
查找步骤元素
  ↓ 找到
检测第一步状态
  ↓
第一步激活？
  ↓ 是                     ↓ 否
显示遮罩层               隐藏遮罩层
  ↓
MutationObserver 监听变化
  ↓
步骤状态改变 → 重新检测 → 更新遮罩显示状态
```

## 🎯 关键检测逻辑总结

### 第一步检测的三种方式

1. **子元素检测**: `firstStep?.querySelector(".is-process") !== null`
2. **头部检测**: `firstStep?.querySelector(".el-step__head.is-process") !== null`
3. **直接类名检测**: `firstStep?.classList.contains("is-process")`

### 多重保障机制

1. **延迟检测**: 等待 Vue 组件渲染（1秒延迟）
2. **重试机制**: 元素未找到时自动重试（500ms 间隔）
3. **DOM 监听**: MutationObserver 实时监听变化
4. **URL 监听**: 定时检查 URL 变化（1秒间隔）

## 💡 优化建议

### 性能优化

1. **防抖处理**: 对频繁触发的检测函数添加防抖
2. **节流处理**: URL 检查可以使用节流而非定时器
3. **选择器缓存**: 缓存常用的 DOM 查询结果

### 用户体验

1. **加载状态**: 显示 GIF 加载状态
2. **关闭按钮**: 允许用户手动关闭遮罩
3. **键盘支持**: ESC 键关闭遮罩
4. **响应式优化**: 移动端适配

### 错误处理

1. **资源加载失败**: 提供占位图或文字提示
2. **网络异常**: 处理 GIF 加载超时
3. **DOM 异常**: 处理元素不存在的情况

## 🔍 调试技巧

### Console 日志

代码中已包含关键日志：
```typescript
console.log("内容脚本已加载 - 移民局扩展");
console.log("遮罩层已显示");
console.log("遮罩层已隐藏");
```

### Chrome DevTools

1. **检查遮罩元素**: `document.getElementById("immigration-bureau-overlay")`
2. **检查步骤状态**: `document.querySelectorAll(".el-step")`
3. **查看消息通信**: Chrome Extensions → Background page → Console

### 测试步骤

1. 打开目标页面
2. 等待页面加载完成
3. 检查 Console 是否有日志输出
4. 检查遮罩元素是否创建
5. 检查 GIF 资源是否加载

## 📚 参考资料

- [Chrome Extensions Manifest V3](https://developer.chrome.com/docs/extensions/mv3/)
- [Content Scripts API](https://developer.chrome.com/docs/extensions/mv3/content_scripts/)
- [MutationObserver API](https://developer.mozilla.org/zh-CN/docs/Web/API/MutationObserver)
- [Element UI Steps 文档](https://element.eleme.cn/#/zh-CN/component/steps)

## 📄 完整代码清单

### content.ts 完整代码

```typescript
// Content Script - 在网页上下文中运行
import "./content.css";

console.log("内容脚本已加载 - 移民局扩展");

// 遮罩层 ID
const OVERLAY_ID = "immigration-bureau-overlay";

// 检查是否是目标页面和第一步
function checkAndShowOverlay() {
  const currentUrl = window.location.href;
  
  // 检查 URL
  if (!currentUrl.includes("entry-registation-form")) {
    return;
  }

  // 等待页面加载完成
  if (document.readyState !== "complete") {
    window.addEventListener("load", checkAndShowOverlay);
    return;
  }

  // 等待 Vue 组件渲染（Element UI）
  setTimeout(() => {
    // 查找第一步（el-step）
    const steps = document.querySelectorAll(".el-step");
    if (steps.length === 0) {
      // 如果还没渲染，再次尝试
      setTimeout(checkAndShowOverlay, 500);
      return;
    }

    const firstStep = steps[0];
    const isFirstStepActive = 
      firstStep?.querySelector(".is-process") !== null ||
      firstStep?.querySelector(".el-step__head.is-process") !== null ||
      firstStep?.classList.contains("is-process");

    if (isFirstStepActive) {
      showOverlay();
    } else {
      hideOverlay();
    }
  }, 1000);
}

// 显示遮罩层
function showOverlay() {
  // 如果已经存在，不重复创建
  if (document.getElementById(OVERLAY_ID)) {
    return;
  }

  const overlay = document.createElement("div");
  overlay.id = OVERLAY_ID;
  overlay.innerHTML = `
    <div class="overlay-content">
      <div class="overlay-text">请刷证件</div>
      <div class="overlay-gif-container">
        <img 
          src="${chrome.runtime.getURL("assets/id-card-scan.gif")}" 
          alt="请刷证件动画"
          class="overlay-gif"
          onerror="this.style.display='none'; this.parentElement.innerHTML='<div style=\\'color: white; font-size: 16px;\\'>GIF 文件未找到，请将 id-card-scan.gif 放置在 src/assets/ 目录下</div>'"
        />
      </div>
    </div>
  `;

  document.body.appendChild(overlay);
  console.log("遮罩层已显示");
}

// 隐藏遮罩层
function hideOverlay() {
  const overlay = document.getElementById(OVERLAY_ID);
  if (overlay) {
    overlay.remove();
    console.log("遮罩层已隐藏");
  }
}

// 使用 MutationObserver 监听步骤变化
function observeStepChanges() {
  const stepsContainer = document.querySelector(".el-steps");
  if (!stepsContainer) {
    setTimeout(observeStepChanges, 500);
    return;
  }

  const observer = new MutationObserver(() => {
    checkAndShowOverlay();
  });

  observer.observe(stepsContainer, {
    childList: true,
    subtree: true,
    attributes: true,
    attributeFilter: ["class"],
  });

  // 初始检查
  checkAndShowOverlay();
}

// 页面加载完成后初始化
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => {
    observeStepChanges();
  });
} else {
  observeStepChanges();
}

// 监听 URL 变化（Vue Router 可能改变 URL）
let lastUrl = window.location.href;
setInterval(() => {
  const currentUrl = window.location.href;
  if (currentUrl !== lastUrl) {
    lastUrl = currentUrl;
    checkAndShowOverlay();
  }
}, 1000);

// 监听来自 background 或 popup 的消息
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message.type === "GET_PAGE_INFO") {
    const pageInfo = {
      title: document.title,
      url: window.location.href,
      timestamp: new Date().toISOString(),
      hasFirstStep: document.querySelector(".el-step .is-process") !== null,
    };
    sendResponse(pageInfo);
  } else if (message.type === "TOGGLE_OVERLAY") {
    if (message.show) {
      showOverlay();
    } else {
      hideOverlay();
    }
    sendResponse({ success: true });
  }
  return true;
});
```

### content.css 完整代码

```css
/* 遮罩层样式 */
#immigration-bureau-overlay {
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background: rgba(0, 0, 0, 0.85);
  z-index: 999999;
  display: flex;
  justify-content: center;
  align-items: center;
  animation: fadeIn 0.3s ease-in-out;
}

@keyframes fadeIn {
  from { opacity: 0; }
  to { opacity: 1; }
}

.overlay-content {
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  gap: 30px;
  text-align: center;
}

.overlay-text {
  font-size: 48px;
  font-weight: bold;
  color: #ffffff;
  text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5);
  font-family: -apple-system, BlinkMacSystemFont, "Microsoft YaHei", "Segoe UI", Roboto, sans-serif;
  animation: pulse 2s ease-in-out infinite;
}

@keyframes pulse {
  0%, 100% {
    transform: scale(1);
    opacity: 1;
  }
  50% {
    transform: scale(1.05);
    opacity: 0.9;
  }
}

.overlay-gif-container {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 100%;
  height: 100%;
  background: rgba(255, 255, 255, 0.1);
  border-radius: 20px;
  padding: 20px;
  box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3);
  backdrop-filter: blur(10px);
}

.overlay-gif {
  max-width: 100%;
  max-height: 100%;
  width: auto;
  height: auto;
  border-radius: 10px;
  animation: scaleIn 0.5s ease-out;
}

@keyframes scaleIn {
  from {
    transform: scale(0.8);
    opacity: 0;
  }
  to {
    transform: scale(1);
    opacity: 1;
  }
}

/* 响应式设计 */
@media (max-width: 768px) {
  .overlay-text {
    font-size: 36px;
  }
  
  .overlay-gif-container {
    width: 300px;
    height: 300px;
  }
}
```

## ✅ 快速集成清单

- [ ] 复制 `content.ts` 到新项目
- [ ] 复制 `content.css` 到新项目
- [ ] 修改目标 URL 检测逻辑
- [ ] 修改步骤检测选择器（根据目标网站调整）
- [ ] 准备 GIF 动画文件
- [ ] 更新 Manifest 配置
- [ ] 配置 `web_accessible_resources`
- [ ] 测试遮罩显示/隐藏功能
- [ ] 测试不同屏幕尺寸
- [ ] 添加错误处理

---

**文档版本**: 1.0  
**最后更新**: 2024-11-02  
**适用版本**: Chrome Extension Manifest V3

